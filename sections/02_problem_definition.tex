...
The central parallel-programming question is how to execute the frame-tile inference tasks across multiple MPI ranks while preserving the serial pipeline output and measuring the cost of distribution. The implementation uses task parallelism, rank mapping as the baseline, node-weighted mapping for the heterogeneous cluster, and duplicate-aware result gathering.
...
Mapping & Baseline rank mapping and node-weighted frame mapping \\
Communication & Direct gather to rank 0, or local merge on each node before rank 0 \\
Load balancing & Tile granularity, node weights, and local rank distribution \\
